...
import { ArrowLeft, Archive, Target, Calendar, Tag, X, ArrowUpDown } from "lucide-react"
...
export default function ArchivedGoalsPage() {
  ...
  const filteredGoals = useMemo(() => {
    if (selectedTags.length > 0) {
      return archivedGoals.filter((goal) => selectedTags.every((tag) => goal.tags.includes(tag)))
    }
    return archivedGoals
  }, [archivedGoals, selectedTags])
        ...
        <div className="mx-auto max-w-6xl px-3 sm:px-6 py-3 sm:py-4">
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-2 sm:gap-3 min-w-0">
              ...
              <div className="flex h-8 w-8 sm:h-10 sm:w-10 items-center justify-center rounded-xl bg-amber-500 flex-shrink-0">
                <Archive className="h-4 w-4 sm:h-5 sm:w-5 text-white" />
              </div>
              <div className="min-w-0">
                <h1 className="text-lg sm:text-xl font-bold text-foreground truncate">Archived</h1>
      ...
      <div className="mx-auto max-w-6xl px-3 sm:px-6 py-4 sm:py-8">
                ...
                {selectedTags.length > 0 && (
                  <div className="mt-2 sm:mt-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                    <p className="text-xs text-muted-foreground">
                      ...
                      <span className="font-medium text-foreground">{archivedGoals.length}</span> goals
                    </p>
        ...
        {archivedGoals.length === 0 ? (
          <div className="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-border bg-card/50 py-12 sm:py-16 px-4">
            <div className="flex h-14 w-14 sm:h-16 sm:w-16 items-center justify-center rounded-full bg-amber-500/10 mb-4">
              <Archive className="h-7 w-7 sm:h-8 sm:w-8 text-amber-600" />
            </div>
            <h3 className="mb-2 text-base sm:text-lg font-semibold text-foreground text-center">No archived goals yet</h3>
            <p className="mb-6 text-center text-sm text-muted-foreground max-w-sm">
              Archive goals to hide them from your main dashboard while keeping them for reference.
            </p>
            <Link href="/">
          ...
          <div className="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-border bg-card/50 py-12 sm:py-16 px-4">
            ...
            <p className="mb-6 text-center text-sm text-muted-foreground max-w-sm">
              No archived goals match the selected tags.
            </p>
